 ...
 *
 * 집계 수치 자체는 이 엔티티에서 직접 관리하지 않는다.
 * 이유:
 * - 반응 저장의 본질은 "누가 어떤 게시글에 어떤 반응을 했는가"라는 조인 데이터 보관이다.
 * - likeCount/scrapCount 같은 집계 값은 조회 최적화용 파생 데이터 성격이 강하다.
 * - 반응 원본 데이터와 집계 데이터를 한 엔티티에 섞으면 조인 이력 보관 책임과 집계 갱신 책임이 뒤섞인다.
 *
 * 그래서 현재 구조는
 * - PostReaction: 반응 원본 데이터 저장
 * - PostStats: 반응 집계와 낙관적 락 관리
 * 로 책임을 나눴다.
 */
@Entity
@Table(
